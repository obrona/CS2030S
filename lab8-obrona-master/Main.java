...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ArrayList<CompletableFuture<BusRoutes>> array = new ArrayList<>();
      while (sc.hasNext()) {
        BusStop srcId = new BusStop(sc.next());
        String searchString = sc.next();
        //System.out.println(BusSg.findBusServicesBetween(srcId, searchString).description());
        array.add(BusSg.findBusServicesBetween(srcId, searchString));
      }
      sc.close();
      CompletableFuture<?>[] actualArray = array.toArray(new CompletableFuture<?>[array.size()]);
      CompletableFuture<Void> allDone = CompletableFuture.allOf(actualArray);
      allDone.join();
      for (int i = 0; i < array.size(); i ++) {
        String s = array.get(i).join().description().join();
