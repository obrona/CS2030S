...
class BusAPI {
   ...
   *     query fails weapped in a string, wrapped in Completable
   *     Future.
   */
  private static CompletableFuture<String> httpGet(String url) {
    ...
    return response.thenApplyAsync(x -> {
      if (x.statusCode() != 200) {
   ...
   *     string if something go wrong wrapped in CompletableFuture
   */ 
   ...
   *     string if the API query failed wrapped in CompletableFuture.
   */ 
